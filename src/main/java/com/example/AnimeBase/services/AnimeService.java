...
public class AnimeService {
    ...
    private static final Pattern NAME_PATTERN = Pattern.compile("^[A-Za-z0-9 ]+$"); // Пример: только буквы и цифры
    private static final Pattern DESCRIPTION_PATTERN = Pattern.compile("^[A-Za-z0-9 .,!?]+$"); // Пример: текст с разрешенными символами
    private static final Pattern TYPE_PATTERN = Pattern.compile("^[A-Za-z ]+$"); // Пример: только буквы и цифры
    //
    ...
    public void saveAnime(Anime anime, MultipartFile file1, MultipartFile file2, MultipartFile file3) throws IOException {
        ...
        log.info("Saving new Anime. Title: {} ; Author: {}", anime.getName(), anime.getDescription());
